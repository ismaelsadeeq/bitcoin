...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def test_old_fee_estimate_file(self):
        ...
        # Restart node to ensure fees/block_policy_estimates.dat file is read
        self.restart_node(0)
        assert_equal(self.nodes[0].estimatesmartfee(1)["feerate"], fee_rate)

        # Stop the node and backdate the block_policy_estimates.dat file more than MAX_FILE_AGE
        block_policy_fee_dat = self.nodes[0].chain_path / "fees/block_policy_estimates.dat"

        # Stop the node and backdate the fees/block_policy_estimates.dat file more than MAX_FILE_AGE
        self.stop_node(0)
        ...
        # Start node and ensure the block_policy_estimates.dat file was not read
        self.start_node(0)
        assert_equal(self.nodes[0].estimatesmartfee(1)["errors"], ["Insufficient data or no feerate found"])


    def test_estimate_dat_is_flushed_periodically(self):
        block_policy_fees_dat = self.nodes[0].chain_path / "fees/block_policy_estimates.dat"
        ...
        # Verify that fees/block_policy_estimates.dat does not exist
        ...
        # Verify that fee estimates were flushed and fees/block_policy_estimates.dat file is created
    ...
    def test_acceptstalefeeestimates_option(self):
        ...
        fee_dat = self.nodes[0].chain_path / "fees/block_policy_estimates.dat"

        # Stop the node and backdate the fees/block_policy_estimates.dat file more than MAX_FILE_AGE
        last_modified_time = time.time() - (MAX_FILE_AGE + 1) * SECONDS_PER_HOUR
        os.utime(fee_dat, (last_modified_time, last_modified_time))

        # Restart node with -acceptstalefeeestimates option to ensure fees/block_policy_estimates.dat file is read
    ...
    def test_estimatesmartfee_return_mempool_estimates(self):
        ...
        block_policy_error = "Insufficient data or no feerate found"
        estimate_after_restart = node0.estimatesmartfee(1)
        verify_estimate_response(estimate_after_restart, None,  [block_policy_error])
    ...
    def run_test(self):
        ...
        self.log.info("Test reading old fees/block_policy_estimates.dat")
