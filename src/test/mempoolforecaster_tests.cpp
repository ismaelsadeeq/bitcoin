...
BOOST_AUTO_TEST_CASE(MempoolEstimator)
{
...

    {
        ...
        BOOST_CHECK(fee_estimate.m_err_message.value() == strprintf("%s: Confirmation target must be greater than zero", MEMPOOL_FORECAST_NAME_STR));
    }

    {
        ...
        BOOST_CHECK(fee_estimate.empty() == true);
        BOOST_CHECK(fee_estimate.m_err_message.value() == strprintf("%s: Confirmation target %s is above maximum limit of %s, mempool conditions might change and forecasts above %s block may be unreliable",
...

    {
        ...
        BOOST_CHECK(fee_estimate.empty() == true);
        BOOST_CHECK(fee_estimate.m_err_message.value() == strprintf("%s: Mempool not finished loading; can't get accurate feerate forecast", MEMPOOL_FORECAST_NAME_STR));
    }

    {
        ...
        const auto fee_estimate = mempool_fee_estimator->EstimateFee(MEMPOOL_FORECAST_MAX_TARGET);
        BOOST_CHECK(fee_estimate.empty() == true);
        BOOST_CHECK(fee_estimate.m_err_message.value() == strprintf("%s: No transactions available in the mempool", MEMPOOL_FORECAST_NAME_STR));
    }
    ...
    // Test when there are not enough mempool transactions to get an accurate estimate
    {
        ...
        const auto fee_estimate = mempool_fee_estimator->EstimateFee(MEMPOOL_FORECAST_MAX_TARGET);
        BOOST_CHECK(fee_estimate.empty() == true);
        BOOST_CHECK(fee_estimate.m_err_message.value() == strprintf("%s: Not enough transactions in the mempool to provide a feerate forecast", MEMPOOL_FORECAST_NAME_STR));
    }
