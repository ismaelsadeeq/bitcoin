...
BOOST_AUTO_TEST_CASE(MempoolUpdated_BlockConnection)
{
    ...
    // Build a CPFB cluster: low-fee parent + high-fee child.
    ...
    {
        LOCK(mpool.cs);
        mpool.removeForBlock({MakeTransactionRef(parent_tx)}, /*nBlockHeight=*/1);
    }
    m_node.validation_signals->SyncWithValidationInterfaceQueue();
...
BOOST_AUTO_TEST_CASE(MempoolUpdated_BlockConflict)
{
    ...
    {
        LOCK(mpool.cs);
        mpool.removeForBlock({MakeTransactionRef(block_tx)}, /*nBlockHeight=*/1);
    }
    m_node.validation_signals->SyncWithValidationInterfaceQueue();
...
// Simulate a scenario where a reorg cause a tx is added to the mempool
// and connect to a cluster, this connection causes mempool limits to burst.
...
// connect the parent->child dependency.
// Here chain[1..cluster_limit] are in-mempool and connects to a tx in block, but the chain
// reorg does not include the parent of chain[1..cluster_limit].
//  chain[0] is then added independently as a cluster of 1. UpdateTransactionsFromBlock scans mapNextTx for
// chain[0]'s outputs, finds chain[1], calls AddDependency — merging both clusters into
// cluster_limit+1 which exceeds the limit. Trim() evicts the lowest-fee tail tx.
