...
int btck_chainstate_manager_import_blocks(btck_ChainstateManager* chainman, const btck_Data* block_file_paths, size_t block_file_paths_len)
{
    try {
        std::vector<fs::path> import_files;
        import_files.reserve(block_file_paths_len);
        for (uint32_t i = 0; i < block_file_paths_len; i++) {
            import_files.emplace_back(std::string{block_file_paths[i].data, block_file_paths[i].len}.c_str());
        }
