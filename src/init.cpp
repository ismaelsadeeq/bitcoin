...
void Shutdown(NodeContext& node)
{
    ...
    node.mempool.reset();
    node.feerate_estimatorman.reset();
    node.chainman.reset();
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    assert(!node.feerate_estimatorman);
    // Don't initialize fee rate estimator manager with old data if we don't relay transactions,
    ...
    if (!peerman_opts.ignore_incoming_txs) {
        ...
        node.feerate_estimatorman = std::make_unique<FeeRateEstimationManager>();
        node.feerate_estimatorman->RegisterFeeRateEstimator(std::make_unique<CBlockPolicyEstimator>(FeeestPath(args), read_stale_estimates));

        // Flush estimates to disk periodically
        FeeRateEstimationManager* fee_estimatorman_ptr = node.feerate_estimatorman.get();
        scheduler.scheduleEvery([fee_estimatorman_ptr] { fee_estimatorman_ptr->IntervalFlush(); }, FLUSH_INTERVAL);
        validation_signals.RegisterValidationInterface(node.feerate_estimatorman.get());
    }
