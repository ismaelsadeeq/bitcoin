                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            auto tx_info = RemovedMempoolTransactionInfo(block.vtx[i], txfee, txVsize, /*dummy height=*/0, /*dummy time*/std::chrono::seconds{0});
    ...
    auto mini_miner_input = GetMiniMinerInput(block_txs);
    auto linearizedResult = node::MiniMiner(std::get<0>(mini_miner_input), std::get<1>(mini_miner_input)).Linearize();
    const auto size_per_feerate = linearizedResult.size_per_feerate;
