...
void MiniMiner::BuildMockTemplate(std::optional<CFeeRate> target_feerate)
{
    ...
    while (!m_entries_by_txid.empty()) {
        ...
        m_linearize_result.size_per_feerate.push_back(std::make_tuple(CFeeRate{ancestor_package_fee, static_cast<uint32_t>(ancestor_package_size)}, static_cast<uint32_t>(ancestor_package_size)));
        ...
        for (const auto& ancestor : ancestors) {
            m_linearize_result.inclusion_order.insert({Txid::FromUint256(ancestor->first), sequence_num});
        }
