...
void clientInvoke(ProxyClient& proxy_client, const GetRequest& get_request, FieldObjs&&... fields)
{
    ...
    proxy_client.m_context.loop->sync([&]() {
        ...
        proxy_client.m_context.loop->logPlain()
            ...
            << TypeName<typename Request::Params>() << " " << LogEscape(request.toString(), proxy_client.m_context.loop->m_log_opts.max_struct_chars);

        proxy_client.m_context.loop->m_task_set->add(request.send().then(
            [&](::capnp::Response<typename Request::Results>&& response) {
                proxy_client.m_context.loop->logPlain()
                    << "{" << thread_context.thread_name << "} IPC client recv "
                    << TypeName<typename Request::Results>() << " " << LogEscape(response.toString(), proxy_client.m_context.loop->m_log_opts.max_struct_chars);
                try {
...
kj::Promise<void> serverInvoke(Server& server, CallContext& call_context, Fn fn)
{
    ...
    server.m_context.loop->log() << "IPC server recv request  #" << req << " "
                                     << TypeName<typename Params::Reads>() << " " << LogEscape(params.toString(), server.m_context.loop->m_log_opts.max_struct_chars);
        ...
        return ReplaceVoid([&]() { return fn.invoke(server_context, ArgList()); },
            ...
            .then([&server, req](CallContext call_context) {
                server.m_context.loop->log() << "IPC server send response #" << req << " " << TypeName<Results>()
                                                 << " " << LogEscape(call_context.getResults().toString(), server.m_context.loop->m_log_opts.max_struct_chars);
            });
