...
<<<<<<< HEAD
void FeeRateForecasterManager::RegisterForecaster(std::unique_ptr<Forecaster> forecaster)
{
...
template<class T>
T* FeeRateForecasterManager::GetForecaster(ForecastType forecaster_type)
{
    Assert(forecasters.contains(forecaster_type));
    Forecaster* forecaster_ptr = forecasters.find(forecaster_type)->second.get();
    return dynamic_cast<T*>(forecaster_ptr);
}

void FeeRateForecasterManager::IntervalFlush()
{
    GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->FlushFeeEstimates();
}

void FeeRateForecasterManager::ShutdownFlush()
{
    GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->Flush();
}

void FeeRateForecasterManager::TransactionAddedToMempool(const NewMempoolTransactionInfo& tx, uint64_t /*unused*/)
{
    GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->processTransaction(tx);
}
...
{
    GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->removeTx(tx->GetHash());
}
...
{
    GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->processBlock(txs_removed_for_block, nBlockHeight);
}

CFeeRate FeeRateForecasterManager::BlockPolicyEstimateRawFee(unsigned int target, double threshold, FeeEstimateHorizon horizon, EstimationResult* buckets)
{
    return GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->estimateRawFee(target, threshold, horizon, buckets);
}

unsigned int FeeRateForecasterManager::BlockPolicyHighestTargetTracked(FeeEstimateHorizon horizon)
{
    return GetForecaster<CBlockPolicyEstimator>(ForecastType::BLOCK_POLICY)->HighestTargetTracked(horizon);
}

ForecastResult FeeRateForecasterManager::ForecastFeeRateFromForecasters(
...
<<<<<<< HEAD
    ...
    for (const auto& [forecast_type, forecast_ptr]  : forecasters) {
        auto curr_forecast = forecast_ptr->ForecastFeeRate(target, conservative);
=======
    LOCK(cs);
...
>>>>>>> c6abe192b8a (fees: introduce lock to forecaster manager and sanity check)
        ...
        error_messages.insert(error_messages.end(), curr_forecast.error_massages.begin(), curr_forecast.error_massages.end());
        // Handle case where the block policy forecaster does not have enough data.
<<<<<<< HEAD
        if (forecast_type == ForecastType::BLOCK_POLICY && curr_forecast.feerate.IsEmpty()) {
            return ForecastResult{.error_massages = error_messages};
=======
...
>>>>>>> c6abe192b8a (fees: introduce lock to forecaster manager and sanity check)
    ...
    feerate_forecast.error_massages = error_messages;
    return feerate_forecast;
}
...
unsigned int FeeRateForecasterManager::MaximumTarget() const
{
    ...
    for (const auto& [_, forecaster] : forecasters) {
        maximum_target = std::max(maximum_target, forecaster->MaximumTarget());
    }
