...
FeeRateEstimatorResult MemPoolFeeRateEstimator::EstimateFeeRate(int target, bool conservative) const
{
    ...
    if (!activeTip) {
        result.errors.emplace_back("No active chainstate available");
        return result;
    }
    result.current_block_height = static_cast<unsigned int>(activeTip->nHeight);
    if (target > MEMPOOL_FEE_ESTIMATOR_MAX_TARGET) {
        result.errors.emplace_back(strprintf("Confirmation target %s exceeds the maximum limit of %s. mempool conditions might change", target, MEMPOOL_FEE_ESTIMATOR_MAX_TARGET));
        return result;
    }
