...
public:
    ...
    void MempoolTxsRemovedForBlock(const std::shared_ptr<const CBlock>& block,
        ...
        EXCLUSIVE_LOCKS_REQUIRED(!cs);
    //! Check if the mempool is healthy based on the recent mined block stats
    // if it's not healthy return INSUFFICIENT_DATA or LOW_COVERAGE, return nullopt if healthy.
