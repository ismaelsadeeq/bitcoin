...
ForecastResult MemPoolLast10MinForecaster::EstimateFeeWithMemPool(unsigned int targetBlocks)
{
    ...
    auto ten_minutes = std::chrono::minutes{10};
    int64_t ten_minutes_in_seconds = std::chrono::duration_cast<std::chrono::seconds>(ten_minutes).count();
    std::vector<std::tuple<CFeeRate, uint64_t>> block_fee_stats = GetNextBlockFeeRateAndVsize(*m_chainstate, m_mempool, ten_minutes_in_seconds);

    if (block_fee_stats.empty()) {
        return ForecastResult(forecast_options, strprintf("%s: No transactions available in the mempool", MEMPOOL_LAST_10_MIN_FORECAST_NAME_STR));
    }
