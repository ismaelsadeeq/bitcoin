...
bitcoin-test-ipc is a test program for interacting with bitcoin-node via IPC.

Usage:
  bitcoin-test-ipc [options]
)"};

static const char* HELP_EXAMPLES{R"(
Examples:
  # Start separate bitcoin-node that bitcoin-test-ipc can connect to.
  bitcoin-node -regtest -ipcbind=unix

  # Connect to bitcoin-node and print tip block hash.
  bitcoin-test-ipc -regtest

  # Run with debug output.
  bitcoin-test-ipc -regtest -debug
)"};
...
static void AddArgs(ArgsManager& args)
{
    ...
    args.AddArg("-echostr=<str>", "Any string that will be echoed before exiting.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
MAIN_FUNCTION
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        std::string output{strprintf("%s bitcoin-test-ipc version", CLIENT_NAME) + " " + FormatFullVersion() + "\n"};
    ...
    std::optional<std::string> echo_str;
    if (args.IsArgSet("-echostr")) {
        echo_str = args.GetArg("-echostr", "");
    }

    // Connect to existing bitcoin-node process or spawn new one.
    std::unique_ptr<interfaces::Init> client_init{interfaces::MakeBasicInit("bitcoin-test-ipc", argc > 0 ? argv[0] : "")};
    assert(client_init);
    std::unique_ptr<interfaces::Init> node_init;
    try {
        std::string address{args.GetArg("-ipcconnect", "unix")};
        node_init = client_init->ipc()->connectAddress(address);
    ...
    tfm::format(std::cout, "Initialize mining interface\n");
    std::unique_ptr<interfaces::Mining> mining{node_init->makeMining()};
    assert(mining);
    tfm::format(std::cout, "Initialize echo interface\n");
    ...
    if (echo_str) {
        tfm::format(std::cout, "Echo string is %s \n", echo->echo(echo_str.value()));
    } else {
        tfm::format(std::cout, "No echo string.\n");
    }
