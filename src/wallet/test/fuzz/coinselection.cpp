...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    if (best_weight < std::numeric_limits<int>::max()) {
        ...
        int64_t high_max_selection_weight = fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(best_weight, std::numeric_limits<int64_t>::max());
    ...
    int64_t low_max_selection_weight = fuzzed_data_provider.ConsumeIntegralInRange<int>(0, best_weight - 1);
