    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            CFeeRate feeRate;
            {
                ...
                feeRate = fee_estimator.EstimateFeeWithMemPool(chainstate, mempool, conf_target);
            }
            UniValue result(UniValue::VOBJ);
            UniValue errors(UniValue::VARR);
            if (feeRate != CFeeRate(0)) {
                ...
                errors.push_back("Insufficient Mempool data");
                result.pushKV("errors", errors);
            }
            return result;
        },
...
static RPCHelpMan estimatesmartfeecons()
{
    return RPCHelpMan{"estimatesmartfeecons",
        "\nEstimates the approximate fee per kilobyte needed for a transaction to begin\n"
        "confirmation within conf_target blocks if possible and return the number of blocks\n"
        "for which the estimate is valid. Uses virtual transaction size as defined\n"
        "in BIP 141 (witness data is discounted).\n",
        ...
        RPCExamples{
            HelpExampleCli("estimatesmartfeecons", "6") +
            HelpExampleRpc("estimatesmartfeecons", "6")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            CBlockPolicyEstimator& fee_estimator = EnsureAnyFeeEstimator(request.context);
            ...
            UniValue errors(UniValue::VARR);
            FeeCalculation feeCalc;
            CFeeRate feeRate{fee_estimator.estimateSmartFeeCons(conf_target, &feeCalc, conservative)};
            if (feeRate != CFeeRate(0)) {
                CFeeRate min_mempool_feerate{mempool.GetMinFee()};
                CFeeRate min_relay_feerate{mempool.m_min_relay_feerate};
                feeRate = std::max({feeRate, min_mempool_feerate, min_relay_feerate});
...
static RPCHelpMan estimaterawfeecons()
{
    return RPCHelpMan{"estimaterawfeecons",
        "\nWARNING: This interface is unstable and may disappear or change!\n"
        "\nWARNING: This is an advanced API call that is tightly coupled to the specific\n"
        "implementation of fee estimation. The parameters it can be called with\n"
        "and the results it returns will change if the internal implementation changes.\n"
        ...
        RPCExamples{
            HelpExampleCli("estimaterawfeecons", "6 0.9")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            CBlockPolicyEstimator& fee_estimator = EnsureAnyFeeEstimator(request.context);
            ...
            for (const FeeEstimateHorizon horizon : ALL_FEE_ESTIMATE_HORIZONS) {
                ...
                feeRate = fee_estimator.estimateRawFeeCons(conf_target, threshold, horizon, &buckets);
                UniValue horizon_result(UniValue::VOBJ);
                UniValue errors(UniValue::VARR);
                UniValue passbucket(UniValue::VOBJ);
                passbucket.pushKV("startrange", round(buckets.pass.start));
...
void RegisterFeeRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        ...
        {"util", &estimatesmartfeecons},
        {"hidden", &estimaterawfeecons},
    };
