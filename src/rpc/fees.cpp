...
static RPCHelpMan estimatesfeewithmempool()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            CMemPoolPolicyEstimator& fee_estimator = EnsureAnyMemPoolFeeEstimator(request.context);
            const NodeContext& node = EnsureAnyNodeContext(request.context);
            const CTxMemPool& mempool = EnsureMemPool(node);

            const int conf_target = request.params[0].getInt<int>();
            CFeeRate feeRate{fee_estimator.EstimateMempoolFeeEstimate(conf_target)};
            UniValue result(UniValue::VOBJ);
            UniValue errors(UniValue::VARR);
            if (feeRate != CFeeRate(0)) {
                CFeeRate min_mempool_feerate{mempool.GetMinFee()};
