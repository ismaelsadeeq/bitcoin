...
FeeRateEstimationManager& EnsureFeeRateEstimatorMan(const NodeContext& node)
{
    if (!node.feerate_estimatorman) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Fee estimation disabled");
    }
    return *node.feerate_estimatorman;
}

FeeRateEstimationManager& EnsureAnyFeeRateEstimatorMan(const std::any& context)
{
    return EnsureFeeRateEstimatorMan(EnsureAnyNodeContext(context));
}
